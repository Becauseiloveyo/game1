...
import { Play, RotateCcw, ChevronRight, Sparkles, Trophy, Star, Lock, Hammer, Hourglass, Dices, Store, Map, BookOpen, LogOut, User, Mail, Key, ShieldAlert } from 'lucide-react';
...
// --- Firebase 专属锁定配置 (修复 [object Object] 解析报错) ---
let firebaseConfig = {
  apiKey: "AIzaSyBN_EHCj66ZW82jS4OSmzo9f3vrzorh0n0",
  authDomain: "game1-333f5.firebaseapp.com",
  projectId: "game1-333f5",
  storageBucket: "game1-333f5.firebasestorage.app",
  messagingSenderId: "426481284730",
  appId: "1:426481284730:web:238e5e8307e776766fedc3",
  measurementId: "G-BYZEFSFQHE"
};

// 兼容预览环境：过滤掉打包工具错误注入的 [object Object]
try {
  if (typeof __firebase_config !== 'undefined' && typeof __firebase_config === 'string') {
    if (__firebase_config !== '[object Object]') {
      firebaseConfig = JSON.parse(__firebase_config);
    }
  }
} catch (e) {
  // 静默处理，使用本地 firebaseConfig
}

const app = initializeApp(firebaseConfig);

// 安全加载 Analytics，防止由于广告拦截器或沙盒环境拦截引发的 Failed to fetch 错误
let analytics = null;
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'crystal-link-game';
...
export default function App() {
  ...
  const [playMode, setPlayMode] = useState(1); // 1: 巡航, 2: 跃迁, 3: 奇点绝境
  const [propsUsedCount, setPropsUsedCount] = useState(0); // 记录单局道具使用次数
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        } else if (isCanvasEnv && !auth.currentUser) {
          await signInAnonymously(auth);
        }
      } catch (e) {
        console.error("Auth init network error:", e);
      }
    };
    initAuth();
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      if (currentUser) setGameState('start');
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists() && !dataLoadedRef.current) {
          const d = docSnap.data();
          setStardust(d.stardust ?? 300);
          setUnlockedLevel(d.unlockedLevel ?? 1);
          setInventory(d.inventory ?? { hammer: 1, addMoves: 1, shuffle: 1 });
          setLevelRecords(d.levelRecords ?? {});
          dataLoadedRef.current = true;
        } else if (!docSnap.exists() && !dataLoadedRef.current) {
          dataLoadedRef.current = true;
        }
      }, (error) => console.warn("存档同步未建立 (可能是网络限制):", error));
      return () => unsubscribe();
    } catch (e) {
  ...
  const saveProgressToCloud = async (newData) => {
    if (!user) return;
    try {
      const docRef = doc(db, 'artifacts', appId, 'users', user.uid, 'saveData', 'main');
  ...
  const getFriendlyErrorMessage = (error) => {
    ...
    if (!code) {
      if (error.message && error.message.includes('fetch')) return '网络请求被拦截，请检查网络或浏览器插件';
      return '发生未知错误，请重试';
    }
    switch (code) {
      ...
      case 'auth/weak-password':
        return '密码太弱，请至少输入6位字符';
      case 'auth/invalid-email':
        return '邮箱格式不正确，请重新输入';
      case 'auth/api-key-not-valid':
        return '系统配置异常 (API Key 无效)，请联系开发者';
      case 'auth/network-request-failed':
        return '网络连接失败，请检查您的网络环境';
      case 'auth/too-many-requests':
        return '尝试次数过多，请稍后重试';
      case 'auth/operation-not-allowed':
        return '该登录方式暂未开启，请联系管理员';
      default:
        // 如果出现未被收录的罕见错误，会去掉 "auth/" 前缀显示，避免出现长串 Firebase 英文
        return `操作失败: ${code.replace('auth/', '')}`;
    }
  };

  const handleAuthAction = async (e) => {
    e.preventDefault(); setAuthError(''); setIsAuthLoading(true);
    if (firebaseConfig.apiKey === "YOUR_API_KEY_HERE") { setAuthError('请先在代码中配置您的 Firebase 参数！'); setIsAuthLoading(false); return; }
    try {
  ...
  const startGameWithMode = (mode) => {
    ...
    setPlayMode(mode);
    setPropsUsedCount(0); // 重置单局道具使用次数
    setScore(0);
  ...
  const isPropDisabled = (propType) => {
    if (playMode === 3) return true; // 炼狱模式锁定
    if (playMode === 2 && propsUsedCount >= 2) return true; // 精英模式限2次
    if (inventory[propType] <= 0) return true; // 数量不足
    return false;
  };
  ...
  const handlePieceClick = async (piece) => {
    ...
    if (activeProp === 'hammer') {
      setIsAnimating(true);
      setActiveProp(null);
      setPropsUsedCount(prev => prev + 1); // 记录使用次数

      const newInventory = { ...inventory, hammer: inventory.hammer - 1 };
      setInventory(newInventory);
      saveProgressToCloud({ inventory: newInventory });
  ...
  const checkWinLoss = () => {
    ...
    if (currentScore >= target) {
      ...
      saveProgressToCloud({ stardust: newDust, unlockedLevel: newUnlockedLevel, levelRecords: newRecords });
  ...
  const buyItem = (item) => {
    ...
    if (stardust >= price) {
      ...
      saveProgressToCloud({ stardust: newDust, inventory: newInventory });
    }
  };
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4 overflow-hidden relative font-sans select-none">
      ...
      {gameState === 'auth' && (
        <div className="z-10 flex flex-col items-center w-full max-w-sm animate-fade-in">
          <div className="relative mb-10 text-center">
            ...
            <p className="text-blue-200 mt-2 tracking-[0.2em] text-sm font-light">登录以同步您的宇宙存档</p>
          </div>
          <form onSubmit={handleAuthAction} className="w-full bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-2xl flex flex-col gap-4">
            ...
            <div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/50" /><input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="舰长邮箱" className="w-full bg-black/30 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-white placeholder-white/40 focus:outline-none focus:border-cyan-400/50 transition-colors" /></div>
            <div className="relative"><Key className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/50" /><input type="password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="安全秘钥 (密码)" className="w-full bg-black/30 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-white placeholder-white/40 focus:outline-none focus:border-cyan-400/50 transition-colors" /></div>
            <button type="submit" disabled={isAuthLoading} className="w-full mt-2 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl font-bold text-lg hover:brightness-110 active:scale-95 transition-all shadow-lg shadow-cyan-500/30">
              {isAuthLoading ? '连接中...' : (authMode === 'login' ? '登 录' : '创建通行证')}
            </button>
            <div className="flex items-center justify-between mt-2 text-sm text-white/60">
              <button type="button" onClick={() => setAuthMode(m => m === 'login' ? 'register' : 'login')} className="hover:text-cyan-300 transition-colors">{authMode === 'login' ? '没有账号？点击注册' : '已有账号？返回登录'}</button>
              <button type="button" onClick={handleGuestLogin} className="flex items-center gap-1 hover:text-white transition-colors"><User className="w-4 h-4" /> 游客模式</button>
            </div>
          </form>
        </div>
      )}
      ...
      {gameState === 'start' && (
        <div className="z-10 flex flex-col items-center w-full animate-fade-in">
          <TopBar title="星空晶莹" showLogout={true} />
                ...
                <div className="flex flex-col gap-3 w-full">
                  ...
                  <button onClick={() => startGameWithMode(3)} disabled={starsUnlocked < 2} className={`group flex items-center justify-between p-4 rounded-xl border transition-all ${starsUnlocked >= 2 ? 'bg-gradient-to-r from-rose-900/40 to-red-900/40 border-rose-500/50 hover:border-rose-400 hover:shadow-[0_0_20px_rgba(244,63,94,0.5)] active:scale-95 cursor-pointer' : 'bg-black/40 border-white/5 opacity-50 cursor-not-allowed'}`}>
                    <div className="text-left">
                      ...
                      <span className={`text-xs tracking-wider mt-1 block ${starsUnlocked >= 2 ? 'text-rose-300/80' : 'text-white/50'}`}>炼狱模式 | 不可用道具</span>
                    </div>
      ...
      {(gameState === 'playing' || gameState === 'won' || gameState === 'lost' || gameState === 'completed') && (
        <div className="w-full max-w-[420px] flex flex-col items-center z-10 animate-slide-up pt-4">
          <div className="w-full flex justify-between items-end px-2 mb-4">
            <button onClick={() => setGameState('levels')} className="p-2 bg-white/10 border border-white/10 rounded-full text-white/70 hover:text-white hover:bg-white/20 transition mb-1">
